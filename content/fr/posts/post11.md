...
tags = ['Hack The Box']
categories = ['Writeups']
images = ['/images/HTB-CozyHosting/CozyHosting.png']
+++
...
CozyHosting est une machine Linux de niveau facile comportant un site web d'hébergement vulnérable à l'injection de commandes.
...
After using the new payload and I get a reverse shell!
...
Search through it with `grep -r password *` and you get some matches. I see that there is a password in `BOOT-INF/classes/application.properties`.
...
We then run `select * from users;`, with this command PostgreSQL will retrieve all rows and all columns from the "users" table and return them as a result set. This query is commonly used to view the contents of a table and can be very useful for inspecting the data stored in the database.
...
Back on the reverse shell, run `ls -la /home` and you will see another directory own by the user `josh`.

![cozyhosting-home-content](/images/HTB-CozyHosting/home-listing.png)

We switch to the `josh` user with `su josh` with the password we just cracked. we then use `cd $home` to get to his home directory and we find the flag `user.txt` there!
...
I go to [https://gtfobins.github.io/gtfobins/ssh/#sudo](https://gtfobins.github.io/gtfobins/ssh/#sudo) to find some ssh root shells commands and I use
