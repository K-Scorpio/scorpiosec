...
This is a medium difficulty Windows machine with a web application accepting user submitted Git repositories URLs.
...
- A PHP application is hosted on the machine
...
We get to the web application by visiting `http://10.10.11.234`. The application appears to be a service allowing user to submit a Git URL, it will then compile the .NET project from for the user.
...
From the application page we gather that our link must lead to a .NET project with a `.sln` file.

![Visual support](/images/HTB-Visual/Visual-dotnet-support.png)

## Foothold

I never used .NET so I asked ChatGPT to show me how create projects for it.
...
This command is used to initialize a new Git repository in the directory.
```
git init
```
...
Back on our server we get some successful requests

![Server good requests](/images/HTB-Visual/server-good-requests.png)

I can see that the application is compiling the files in the git repo. We need to find a way to execute commands before the build starts. I researched it and found this Microsoft page [Specify build events (C#)](https://learn.microsoft.com/en-us/visualstudio/ide/how-to-specify-build-events-csharp?view=vs-2022) explaining that a "pre-build event"  refers to a script or command that is executed before the build process for the project begins.

We can use the `PreBuild` and `PostBuild` in the `.csproj file` to execute some commands. So let's try to add a reverse shell to the file. 

- This is my modified .csproj file
```
...
If you stopped your Python server start it again (and leave it running)
...
Running `whoami` lets us know that we are user `enox`
...
I start exploring the user directories with `cd \Users\enox` and I use `ls` to see the different directories.

![directories listing on target](/images/HTB-Visual/target-directories.png)

I check what is in the `Desktop` directory with `cd Desktop` and I can see the `user.txt` flag. Use `cat user.txt` to check its content.
...
Now you get it to the target with `wget`
```
